...
function createInputVector(spec, tasks, stage1, viewConnectionType) {
    ...
    inputArray.push(
        (inputVectorObject['d_sparseinterconnection'] = sparseInterconnection && featureInterconnection ? 1 : 0)
    );
    inputArray.push(
    ...
    inputArray.push(
        (inputVectorObject['s_color'] =
            channels.includes('color(sequential)') || channels.includes('color(nominal)') ? 1 : 0)
    );
...
function getLayoutUpdated(visOptions, tasks, viewConnectionType) {
    ...
    visOptions.forEach(element => {
        ...
        recommendation.forEach(rec => {
                ...
                const encodings = [
                    {
                        ...
                        encodingPredictionScore: track['encodingPredictionScore'],
            ...
            var tempOutput = {
                trackAlignment: element['trackAlignment'],
                trackAlignmentPrediction: element['trackAlignmentPrediction'],
                tracks: tracksTemp
            };
